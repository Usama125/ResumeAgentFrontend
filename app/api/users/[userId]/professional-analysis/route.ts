...
export async function GET(
  ...
  try {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/admin/analytics/track`, {
        ...
        body: JSON.stringify({
          ...
          username: null,
          details: {
            analysis_type: 'professional_analysis',
    ...
    // Forward to backend
    const backendUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/users/${userId}/professional-analysis`;
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: 'Failed to fetch professional analysis' },
