...
interface PublicationsSectionEditModalProps {
  isOpen: boolean
  onClose: () => void
  currentPublications: Publication[]
  onUpdate: (newPublications: Publication[]) => void
  editingPublication?: Publication | null
  editingIndex?: number | null
  mode: 'add' | 'edit'
}

export default function PublicationsSectionEditModal({
  isOpen,
  onClose,
  currentPublications,
  onUpdate,
  ...
  editingIndex,
  mode = 'add'
}: PublicationsSectionEditModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (mode === 'edit' && editingPublication) {
        setPublication({ ...editingPublication })
      } else {
        setPublication({ title: "", publisher: "", date: "", url: "", description: "" })
      }
    }
  }, [isOpen, mode, editingPublication])

  const isFormValid = () => {
    return publication.title.trim() !== ""
  }

  const hasChanges = () => {
    if (mode === 'add') return true
    if (!editingPublication) return false
    
    return (
      publication.title !== editingPublication.title ||
      publication.publisher !== editingPublication.publisher ||
      publication.date !== editingPublication.date ||
      publication.url !== editingPublication.url ||
      publication.description !== editingPublication.description
    )
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      let updatedPublications = [...currentPublications]
      
      if (mode === 'add') {
        updatedPublications.unshift(publication)
      ...
      toast({
        title: "Success",
        description: mode === 'add' ? "Publication added successfully" : "Publication updated successfully",
      })
      onClose()
    } catch (error: any) {
      console.error(`Error ${mode === 'add' ? 'adding' : 'updating'} publication:`, error)
      toast({
        title: "Error",
        description: error.message || `Failed to ${mode === 'add' ? 'add' : 'update'} publication`,
        variant: "destructive"
      })
    } finally {
  ...
  const handleInputChange = (field: keyof Publication, value: string) => {
          ...
          <div className={`shrink-0 p-6 border-b ${isDark ? 'border-[#10a37f]/20' : 'border-gray-200'}`}>
            <div className="flex items-center justify-between">
              <h2 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {mode === 'add' ? 'Add New Publication' : 'Edit Publication'}
              </h2>
              <button
                onClick={onClose}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Publication date (e.g., January 2024)"
                  className={`${isDark ? 'bg-[#1a1a1a] border-[#10a37f]/30 text-white placeholder:text-gray-400' : 'bg-white border-gray-200 text-gray-900 placeholder:text-gray-500'}`}
                />
              </div>
